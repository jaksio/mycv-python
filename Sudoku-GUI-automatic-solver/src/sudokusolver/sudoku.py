    ...
    for i in range(0, 9):
        for j in range(0, 9):
            pygame.draw.rect(
                ...
                white,
                pygame.Rect(
                    border + (i * rec_size), border + (j * rec_size), rec_size, rec_size
                ),
                2,
            )
            pygame.display.flip()
...
def fill_board(board: list[list[int]]):
...
def check_number(board: list[list[int]], user_in: int, a: int, b: int):
    ...
    for i in board[a * 3: a * 3 + 3]:
        for j in i[b * 3: b * 3 + 3]:
...
def clear_rec_by_cols(board: list[list[int]], a: int, b: int) -> None:
...
def clear_rec(
    board: list[list[int]], x: int, y: int, not_delete: list[tuple[int, int]]
) -> list[list[int]]:
    ...
    myRec = pygame.Rect(x, y, rec_size, rec_size)
...
def select_rectangle(x: int, y: int, prev_color: str, prev_position):
    ...
    prevRec = pygame.Rect(prev_position[0], prev_position[1], rec_size, rec_size)
    myRec = pygame.Rect(
